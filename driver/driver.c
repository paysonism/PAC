...
UNICODE_STRING g_DeviceName         = RTL_CONSTANT_STRING(L"\\Device\\PAC");
UNICODE_STRING g_DeviceSymbolicLink = RTL_CONSTANT_STRING(L"\\??\\PAC");
...
{
        return g_DriverConfig->has_winlogon_started; // payson1337
}
...
DriverEntry(_In_ PDRIVER_OBJECT DriverObject, _In_ PUNICODE_STRING RegistryPath)
{
        ...
        DEBUG_INFO("Driver Entry Complete - github.com/paysonism");
        return STATUS_SUCCESS;
}
